...
API_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MjA1OTQ0NTMsImlhdCI6MTcyMDQyNjQ1MywiaXNzIjoiL2FwaS92MS9hdXRoL2Fub255bW91cyIsInN1YiI6ImRlMmY1NTIxM2IyOGJlMWQzMTY0OTY2MzlmOTA0NWJkMzM3MTU5YzNhMDM1ZDY0YTc1NzM4YjY1ODI2MzIwY2YiLCJ0eXBlIjoxMH0.f3Q41ZdwvGlLVsBP-hzpjOilDRZge7Qunl9lE8xq8oM"
...
def test_get_book_by_id():
    ...
    response = requests.get(
        f"{BASE_URL}/products/slug/{book_id}", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_search_books_rus():
    ...
    response = requests.get(
        f"{BASE_URL_2}/search/product?phrase=Гарри Поттер", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_search_books_eng():
    ...
    response = requests.get(
        f"{BASE_URL_2}/search/product?phrase=Harry Potter", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_get_category_books():
    ...
    response = requests.get(
        f"{BASE_URL_2}/products?forceFilters[categories]=110227", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_get_filtres_books():
    ...
    response = requests.get(
        f"{BASE_URL_2}/products?filters[onlyNew]=1", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_get_books_by_author():
    ...
    response = requests.get(
        f"{BASE_URL_2}/products/facet?filters[authors]=593125", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_add_book_to_cart():
    ...
    response = requests.post(
        f"{BASE_URL}/cart/product", json=data, headers=headers)
    assert response.status_code == 400, f"Ожидался статус-код 200, но получен {
        response.status_code}"
...
def test_del_book_from_cart():
    ...
    assert response.status_code == 204, f"Ожидался статус-код 200, но получен {
        response.status_code}"
